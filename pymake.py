...
def compileObjs(settings, dObjFiles):
    ...
    for sObj, sDepen in dObjFiles.items():
        ...
        if os.system(sBaseCmd + sMid + sEndCmd):
            print('Failed to compile ' + sObj)
            exit(1)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(settings['Bin']):
        os.makedirs(settings['Bin'])

    if not os.path.exists(settings['Obj Dir']):
        os.makedirs(settings['Obj Dir'])
    ...
    setsSources = {
        ...
        for sFile in findSourceFiles(sDir, ['.c', '.cpp', '.h', '.hpp'])
    }
